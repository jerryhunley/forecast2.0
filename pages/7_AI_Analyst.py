...
4.  **Time-Series Analysis (Rate Trends):** To calculate a rate trend over time, you must calculate monthly totals for the numerator and the denominator separately, combine them, then divide.
    *   **Correct Pattern:**
        ```python
...
"""
    prompt += _df_info
    prompt += "-----------------------------\n\n"
    prompt += "Your response MUST be ONLY the Python code block, starting with ```python and ends with ```."
    
    return prompt

@st.cache_data
def get_summarizer_prompt():
    # This prompt is for the second API call: generating the summary.
