...
def get_coder_prompt(_df_info, _ts_col_map_str):
    # This is our existing, highly-tuned prompt for generating code for a SINGLE step.
    return f"""You are a world-class Python data analyst. Your goal is to answer a SINGLE, simple question by generating a single, executable Python code block.
...
4.  **Time-Series Analysis (Rate Trends):** To calculate a rate trend over time, calculate monthly totals for the numerator and the denominator separately, then combine them before dividing.
...
- `ts_col_map`: A dictionary mapping stage names to timestamp columns: `{_ts_col_map_str}`
