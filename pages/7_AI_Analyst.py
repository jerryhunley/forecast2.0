...
def get_coder_prompt(_df_info, _ts_col_map_str):
    prompt_parts = [
        ...
        "1.  **Primary Date Column:** For any general time-based filtering (like 'most recent month', 'last 90 days', etc.), you MUST use the column named `'Submitted On_DT'`. This is the most important rule.",
        "2.  **DataFrame Display:** The `df` contains complex columns ('Parsed_Lead_Stage_History', 'Parsed_Lead_Status_History') that cannot be displayed. Before using `st.dataframe()`, you MUST explicitly drop these 'Parsed_' columns.",
        "3.  **Time-Series Analysis (Resampling):** To count events \"by week\" or \"by month\", you must use `pd.Grouper`. DO NOT use `.set_index().resample()`. The correct pattern is to group by `pd.Grouper(key='YOUR_TIMESTAMP_COLUMN', freq='W')` for weekly or `'M'` for monthly.",
        "    *   **Correct Pattern:** `df.groupby(pd.Grouper(key=ts_col_map['Sent To Site'], freq='W')).size()`",
        "    *   **INCORRECT (Forbidden) Pattern:** `df.set_index(ts_col_map['Sent To Site']).resample('W').size()`",
        "4.  **Time-Series Analysis (Rate Trends):** To calculate a rate trend over time, you must calculate the monthly totals for the numerator and the denominator separately using the correct `pd.Grouper` pattern, then combine them before dividing.",
